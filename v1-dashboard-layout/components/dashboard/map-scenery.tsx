...
export function MapScenery({ weather = 'sunny', showAnimated = true }: MapSceneryProps) {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      ...
      {/* Sun */}
      {weather === 'sunny' && (
        <div className="animate-sun-fade absolute top-12 right-16 h-20 w-20 rounded-full bg-yellow-300 shadow-lg shadow-yellow-400/50" />
      )}

      {/* Moon */}
      {weather === 'rain' && (
        <div className="animate-moon-glow absolute top-16 right-20 h-16 w-16 rounded-full bg-gray-100 opacity-60" />
      )}
        ...
        {[0, 1, 2, 3].map((i) => (
          ...
          >
            <svg width="120" height="50" viewBox="0 0 120 50" fill="none">
              <path
                d="M 20 35 Q 10 30 10 20 Q 10 10 20 8 Q 25 2 35 5 Q 45 0 55 8 Q 70 5 75 20 Q 85 15 95 20 Q 100 25 95 35 Z"
                fill="white"
      ...
      {/* Birds flying - only show in sunny weather */}
      {weather === 'sunny' && (
      ...
      {weather === 'rain' && (
        <div className="absolute inset-0 pointer-events-none">
          {[...Array(30)].map((_, i) => (
            <div
              key={`rain-${i}`}
              className="animate-rain-fall absolute w-1 h-8 bg-gradient-to-b from-blue-300 to-transparent rounded-full"
              style={{
      ...
      {weather === 'snow' && (
        <div className="absolute inset-0 pointer-events-none">
          {[...Array(40)].map((_, i) => (
            <div
              key={`snow-${i}`}
              className="animate-snowflake-fall absolute"
              style={{
                left: `${Math.random() * 100}%`,
      ...
      {/* Rainbow - appears after rain or completing difficult quest */}
      {weather === 'rainbow' && (
      ...
      {/* Mountains in background - stationary but layered for depth */}
