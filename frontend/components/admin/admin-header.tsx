...
export function AdminHeader({ onMenuClick }: AdminHeaderProps) {
  ...
  const showAvatarImage =
    Boolean(avatarSrc) && !isDefaultOrEmptyAvatar(user?.avatar_url) && !avatarLoadFailed;
  ...
  return (
    <header className="coach-header-bar sticky top-0 z-30 flex h-14 shrink-0 items-center justify-between border-b border-sidebar-border px-4 sm:px-6">
      <div className="flex min-w-0 items-center gap-3">
        ...
        >
          <Shield className="h-5 w-5 shrink-0 text-amber-300/90" aria-hidden />
      ...
      <div className="relative flex items-center" ref={userMenuRef}>
        <button
          ...
          aria-haspopup="true"
        >
          <div className="sidebar-avatar relative flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full border-2 border-amber-400/80 bg-white/10 text-sm font-bold text-sidebar-foreground">
