        ...
        <div className="flex gap-2 overflow-x-auto border-b border-gray-200 bg-gray-50 p-4 scrollbar-hide">
          {filters.map((filter) => (
            <button
              ...
              className={`whitespace-nowrap rounded-full px-4 py-2 font-bold transition-all ${activeFilter === filter ? 'bg-blue-500 text-white shadow-md' : 'bg-white text-gray-700 border border-gray-200 hover:border-blue-300'}`}
            >
              {filter}
            </button>
          ))}
        ...
        <div className="divide-y divide-gray-100 p-4">
          {filteredActivities.length > 0 ? (
            filteredActivities.map((activity, idx) => (
              <div key={activity.id} className="animate-activity-slide-in flex items-start gap-4 py-4 transition-all hover:bg-gray-50 rounded-lg px-2" style={{ animationDelay: `${idx * 0.1}s` }}>
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gray-100">{activity.icon}</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="font-heading text-base font-bold text-gray-900 truncate">{activity.title}</p>
                    <span className={`inline-flex shrink-0 rounded-full px-2.5 py-0.5 text-xs font-bold animate-badge-flip ${activity.badgeColor}`}>{activity.badge}</span>
                  </div>
                  {activity.subtitle && <p className="text-sm font-semibold text-gray-500">{activity.subtitle}</p>}
                  {activity.stats && <p className="text-xs font-semibold text-gray-500 mt-1">{activity.stats}</p>}
                ...
                <div className="flex shrink-0 flex-col items-end gap-2">
                  ...
                  {activity.actionUrl === '/puzzles' ? (
                    <Link href="/puzzles" className="flex items-center gap-1 rounded-lg bg-blue-100 px-3 py-1.5 text-xs font-bold text-blue-700 hover:bg-blue-200 transition-colors">
                      {activity.action === 'Try Similar' ? <><RotateCcw className="h-3 w-3" /> Try Similar</> : <><span>{activity.action}</span> <ArrowRight className="h-3 w-3" /></>}
                    </Link>
                  ) : (
                    <button className="flex items-center gap-1 rounded-lg bg-blue-100 px-3 py-1.5 text-xs font-bold text-blue-700 hover:bg-blue-200 transition-colors">
                      {activity.action === 'Share' ? <><Share2 className="h-3 w-3" /> Share</> : <><span>{activity.action}</span> <ArrowRight className="h-3 w-3" /></>}
                    </button>
                  )}
                </div>
              </div>
            ...
            <div className="flex flex-col items-center justify-center gap-3 py-12 text-center">
              ...
              <Link href="/puzzles" className="mt-2 rounded-lg bg-blue-500 px-6 py-2 font-bold text-white hover:bg-blue-600 transition-colors">Start Playing!</Link>
            </div>
          )}
        </div>

        <div className="border-t border-gray-200 bg-gray-50 p-4">
          <button onClick={handleLoadMore} disabled={loadingMore} className="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-blue-300 py-3 font-bold text-blue-600 hover:bg-blue-50 transition-colors disabled:opacity-50">
            {loadingMore ? <><div className="animate-spinner h-4 w-4 border-2 border-blue-600 border-t-transparent rounded-full" />Loading...</> : <><ChevronRight className="h-5 w-5" />Load More Activities</>}
          </button>
