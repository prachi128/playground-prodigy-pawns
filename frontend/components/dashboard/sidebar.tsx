...
const navItems = [
  { label: "Home", icon: Gamepad2, href: "/dashboard", active: true, badge: null, progress: null },
  { label: "Adventure", icon: Map, href: "/dashboard", active: false, badge: null, progress: null },
  { label: "Play", icon: Swords, href: "/dashboard", active: false, badge: null, progress: null },
  { label: "Puzzles", icon: Puzzle, href: "/puzzles", active: false, badge: null, progress: null },
  { label: "Learn", icon: GraduationCap, href: "/dashboard", active: false, badge: null, progress: null },
  { label: "Progress", icon: TrendingUp, href: "/dashboard", active: false, badge: null, progress: null },
  { label: "Leaderboard", icon: Star, href: "/leaderboard", active: false, badge: null, progress: null },
  { label: "Friends", icon: Users, href: "/dashboard", active: false, badge: null, progress: null },
  { label: "Class", icon: Users2, href: "/dashboard", active: false, badge: null, progress: null },
]
...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const xpForLevel = 500 + (level - 1) * 250
  const xpProgress = Math.min(100, Math.round((totalXP % xpForLevel) / xpForLevel * 100))
    ...
    <>
      ...
      >
        ...
        <div className={`transition-all duration-300 ${collapsed ? "px-2 py-3" : "px-3 py-4"}`}>
          <div className={`overflow-hidden rounded-xl bg-gradient-to-br from-white/10 to-white/5 shadow-lg transition-all duration-300 ${collapsed ? "p-2" : "p-4"}`}>
            <div className="flex items-center gap-3">
              <div className="sidebar-avatar relative h-16 w-16 shrink-0 overflow-hidden rounded-xl border-4 border-yellow-400 shadow-lg">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1.5 flex items-center gap-3">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-base font-bold text-yellow-400">{user?.rating ?? 0}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <Flame className="h-5 w-5 text-orange-400" />
                      <span className="text-base font-bold text-orange-400">0</span>
                    </div>
                  </div>
                </div>
              )}
            ...
            {!collapsed && (
              <div className="mt-4">
                <div className="mb-2 flex items-center justify-between text-sm font-bold">
                  ...
                  <span className="text-yellow-400">{totalXP % xpForLevel} / {xpForLevel}</span>
                </div>
        ...
        <nav className="flex-1 overflow-y-auto">
          <ul className={`sidebar-nav-list flex flex-col transition-all duration-300 ${collapsed ? "px-2 py-2 space-y-2" : "px-3 py-2 space-y-3"}`} role="list">
            {navItems.map((item) => {
        ...
        <div className={`border-t-2 border-sidebar-border bg-sidebar-accent/30 transition-all duration-300 ${collapsed ? "px-2 py-3" : "px-3 py-4"}`}>
          ...
          <button onClick={handleLogout} className="group flex w-full items-center gap-3 rounded-xl px-4 py-4 text-base font-medium text-sidebar-foreground/80 transition-all hover:scale-105 hover:bg-red-500/30 hover:text-red-200" title={collapsed ? "Log Out" : undefined}>
