// app/(coach)/admin/students/[id]/page.tsx — Admin student profile
...
import { coachStudentApiRef } from '@/lib/coach-student-path';
import { adminStudentReportPath } from '@/lib/admin-student-path';
...
export default function AdminStudentDetailPage() {
  ...
  const [coaches, setCoaches] = useState<Array<{ id: number; username: string; full_name: string; is_active?: boolean }>>([]);
  const [assignBusy, setAssignBusy] = useState(false);
  const [assignedCoachId, setAssignedCoachId] = useState<number | null>(null);
  ...
  const loadCoaches = async () => {
    try {
      ...
      if (user) {
        ...
        setCoaches(
          loaded.some((c: { id: number }) => c.id === adminOption.id)
            ? loaded
            : [adminOption, ...loaded],
        );
      } else {
  ...
  const loadStudent = async () => {
    ...
    setIsLoading(true);
    try {
      const [response, adminListRes] = await Promise.all([
        api.get(`/api/coach/students/${coachStudentApiRef(studentUsername)}`),
        api.get('/api/admin/students'),
      ]);
      const detail = response.data as StudentDetails;
      setStudent(detail);
      const rows = Array.isArray(adminListRes.data) ? adminListRes.data : [];
      const row = rows.find((r: { id: number; username: string }) => r.username === detail.username);
      setAssignedCoachId(row?.coach_id ?? null);
    } catch {
  ...
  const handleReactivate = async () => {
    ...
    try {
      await api.put(`/api/admin/students/${student.id}/reactivate`);
  ...
  const assignCoach = async (rawValue: string) => {
    if (!student) return;
    ...
    setAssignBusy(true);
    try {
      await api.put(`/api/admin/students/${student.id}/assign-coach`, { coach_id: coachId });
      ...
      <div className={`${panel} space-y-4`}>
        <div className="flex flex-wrap items-end justify-between gap-3 rounded-lg border border-border bg-muted/20 p-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Assign which coach owns this student&apos;s roster.
            </p>
          </div>
          <div className="flex items-center gap-2">
            <select
              onChange={(e) => void assignCoach(e.target.value)}
              disabled={assignBusy}
              className="rounded-lg border border-border bg-background px-3 py-2 text-sm"
              value={assignedCoachId != null ? String(assignedCoachId) : '__none__'}
            >
            ...
            {assignBusy ? <Loader2 className="h-4 w-4 animate-spin text-primary" /> : null}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex shrink-0 flex-wrap gap-2">
            ...
            <button
              type="button"
              onClick={() =>
                window.open(adminStudentReportPath(student.username), '_blank', 'noopener,noreferrer')
              }
              className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-background px-3 py-2 text-[13px] font-medium text-foreground hover:bg-muted/60"
            >
        ...
        {!isStudentActive && (
          ...
          >
            ...
            <p>Account deactivated — student cannot sign in until reactivated.</p>
          </div>
        )}
