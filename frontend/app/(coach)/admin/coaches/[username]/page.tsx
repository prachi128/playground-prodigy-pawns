// app/(coach)/admin/coaches/[username]/page.tsx — Admin coach activity detail
...
import { adminCoachApiRef, adminCoachesPath } from '@/lib/admin-coach-path';
...
function lastLoginClass(days: number | null | undefined): string {
  ...
  if (days <= 1) return 'text-[hsl(var(--green-medium))]';
...
export default function AdminCoachDetailPage() {
  ...
  const coachUsername = decodeURIComponent(params.username ?? '');
  ...
  const loadCoach = useCallback(async () => {
    if (!coachUsername) {
      router.replace(adminCoachesPath());
      return;
    }
    setIsLoading(true);
    try {
      const data = await adminAPI.getCoachActivity(adminCoachApiRef(coachUsername));
      setCoach(data);
    } catch {
  ...
  if (!coach) {
    return (
      <div className="py-16 text-center">
        ...
        <Link href={adminCoachesPath()} className="mt-4 inline-block text-sm font-semibold text-primary">
          Back to coaches
        </Link>
      </div>
    );
