...
export default function StarCollectorPage() {
  return (
    <div className="mx-auto max-w-6xl pt-6">
          ...
          <div className="relative flex-1">
            ...
            <div className="rounded-2xl bg-card p-4 shadow-sm">
              ...
              <p className="mt-0.5 font-heading text-sm font-semibold text-muted-foreground">
                Collect every star by moving one piece. Choose a piece to start! ⭐
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Piece cards - dashboard card style with amber/star gradient */}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {PIECE_ORDER.map((pieceType, idx) => {
            ...
            return (
              <Link
                key={pieceType}
                href={`/learn/star-collector/${slug}`}
                className="animate-bounce-in hover-wiggle group flex items-center gap-4 overflow-hidden rounded-3xl border-2 border-amber-300 bg-white p-4 shadow-md transition-all duration-200 hover:-translate-y-1 hover:shadow-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                style={{ animationDelay: `${idx * 80}ms` }}
              >
                <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-amber-400 to-yellow-500 text-2xl font-bold text-white shadow-sm transition-transform duration-300 group-hover:scale-105">
                  {meta.pieceName[0]}
                </div>
