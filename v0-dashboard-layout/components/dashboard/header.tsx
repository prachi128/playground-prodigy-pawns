...
const INITIAL_NOTIFICATIONS: Notification[] = [
  {
    ...
    title: "Puzzle Pro Unlocked!",
    message: "You solved 100 puzzles -- amazing work!",
    time: "2 min ago",
    dateGroup: "today",
    read: false,
  },
  {
    ...
    title: "Coach Alex says...",
    message: "Great job on Knight forks! Watch the Rook Endgames video next.",
    time: "15 min ago",
    dateGroup: "today",
    read: false,
  },
  {
    ...
    message: "Advanced Tactics: The Isolated Queen Pawn is ready for you.",
    time: "1 hour ago",
    dateGroup: "today",
    read: false,
  },
  {
    ...
    title: "7-Day Streak!",
    message: "You played 7 days in a row. Keep going for a bonus reward!",
    time: "Yesterday",
    dateGroup: "yesterday",
    read: true,
  },
  {
    ...
    icon: <Swords className="h-5 w-5 text-emerald-500" />,
    title: "New Challenge!",
    message: "Coach Alex sent you a Mate-in-2 challenge. Can you solve it?",
    time: "Yesterday",
    dateGroup: "yesterday",
    read: true,
  },
  {
    ...
    title: "System Update",
    message: "New board themes and piece sets are available in the Star Shop!",
    time: "3 days ago",
    dateGroup: "this_week",
    read: true,
  },
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-3 bg-gradient-to-r from-emerald-600 to-green-500 px-4 shadow-md lg:px-6">
      ...
      <div className="flex items-center gap-2">
        <span className="animate-mascot-bounce text-2xl" role="img" aria-label="wave">
          {"👋"}
        </span>
      ...
      <div className="ml-auto flex items-center gap-3">
        ...
        <div className="flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-2 transition-transform hover:scale-105">
          ...
          <span className="hidden font-heading text-sm font-bold text-white sm:inline">640 XP</span>
        </div>

        {/* Streak badge */}
        <div className="flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-2 transition-transform hover:scale-105">
          <Flame className="h-5 w-5 animate-flame text-orange-300" />
          <span className="hidden font-heading text-sm font-bold text-white sm:inline">7</span>
        </div>

        {/* Stars */}
        <div className="flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-2 transition-transform hover:scale-105">
          <Star className="h-4 w-4 fill-yellow-300 text-yellow-300" />
          <span className="hidden font-heading text-sm font-bold text-white sm:inline">42</span>
        </div>
        ...
        <div className="relative" ref={dropdownRef}>
          <button
            ...
            className="relative rounded-xl p-2.5 text-white/80 transition-all hover:scale-110 hover:bg-white/10 hover:text-white"
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className="animate-badge-pulse absolute -right-0.5 -top-0.5 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white ring-2 ring-emerald-600">
                {unreadCount}
              </span>
            )}
          </button>
          ...
          {/* ── Dropdown ── */}
          {isOpen && (
            <div className="animate-dropdown-slide absolute right-0 top-full mt-2 w-[calc(100vw-20px)] overflow-hidden rounded-2xl border border-border bg-card shadow-2xl sm:w-[400px]"
              style={{ zIndex: 50 }}
            >
              {/* Dropdown header */}
                ...
                {TABS.map((tab) => {
                  ...
                  return (
                    ...
                    >
                      {tab.label}
                      {tabCount > 0 && (
                        <span className="ml-1.5 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-red-100 px-1 text-[10px] font-bold text-red-600">
                          {tabCount}
                        </span>
                      )}
                      {isActive && (
                      ...
                      <div key={group}>
                        ...
                        {items.map((notification) => {
                          ...
                          return (
                            ...
                            >
                              {/* Delete backdrop (swipe reveal) */}
                              <div className="absolute inset-y-0 right-0 flex w-24 items-center justify-center bg-red-500 text-sm font-bold text-white">
                                Dismiss
                              </div>
                              ...
                              <button
                                ...
                                className={`relative flex w-full items-start gap-3 px-4 py-3 text-left transition-all hover:bg-emerald-50/60 ${
                                  !notification.read ? "bg-blue-50/50" : "bg-card"
                                }`}
                                style={{
                              ...
                              >
                                ...
                                <div className="min-w-0 flex-1">
                                  ...
                                  <p className="mt-1 text-[10px] font-medium text-muted-foreground/50">
                                    {notification.time}
                                  </p>
                                </div>
                              </button>
              ...
              <div className="border-t border-border bg-card px-4 py-3 text-center">
                <button className="font-heading text-sm font-bold text-emerald-600 transition-colors hover:text-emerald-700">
