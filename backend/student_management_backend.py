...
def _safe_email(user: User) -> str:
    """List/detail schemas expect a string even for legacy rows with NULL email."""
...
class CoachActivityRow(BaseModel):
    ...
    attendance_marked_total: int
    attendance_marked_this_week: int
    announcements_total: int
    batch_names: List[str]


class CoachDetailedActivity(CoachActivityRow):
    batches: List[CoachActivityBatchRow]
    recent_activity: List[CoachRecentActivityRow]
...
def _is_assignable_coach(user: User) -> bool:
    return user.role in (UserRole.coach, UserRole.admin, "coach", "admin")


def _get_assignable_coach(db: Session, coach_id: int) -> Optional[User]:
    coach = db.query(User).filter(User.id == coach_id).first()
    if not coach or not _is_assignable_coach(coach):
        return None
    return coach
...
def _days_since_login(last_login: Optional[datetime]) -> Optional[int]:
    if last_login is None:
        return None
    return max(0, (datetime.utcnow() - last_login).days)
...
def _resolve_coach_ref(db: Session, coach_ref: str) -> User:
...
def _enrolled_student_maps(
    db: Session, coach_ids: List[int]
) -> Tuple[dict[int, Set[int]], dict[int, Set[int]]]:
    """Per coach: all enrolled student ids and subset with active accounts."""
    total_map: dict[int, Set[int]] = defaultdict(set)
    active_map: dict[int, Set[int]] = defaultdict(set)
    if not coach_ids:
        return total_map, active_map
    rows = (
        db.query(Batch.coach_id, StudentBatch.student_id, User.is_active)
    ...
    for coach_id, student_id, is_active in rows:
        total_map[int(coach_id)].add(int(student_id))
        if is_active:
            active_map[int(coach_id)].add(int(student_id))
    return total_map, active_map


def _build_coach_activity_maps(db: Session, coach_ids: List[int]) -> dict:
    """Aggregate activity metrics for many coaches in a small number of queries."""
    if not coach_ids:
        return {}

    week_ago = _week_ago()
    metrics: dict[int, dict] = {cid: defaultdict(int) for cid in coach_ids}
    batch_names: dict[int, List[str]] = defaultdict(list)

    batch_rows = (
        db.query(
            Batch.coach_id,
            func.count(Batch.id),
            func.sum(case((Batch.is_active == True, 1), else_=0)),
        )
        .filter(Batch.coach_id.in_(coach_ids))
        .group_by(Batch.coach_id)
        .all()
    )
    for coach_id, total, active in batch_rows:
        metrics[int(coach_id)]["total_batches"] = int(total or 0)
        metrics[int(coach_id)]["active_batches"] = int(active or 0)

    name_rows = (
        db.query(Batch.coach_id, Batch.name)
        .filter(Batch.coach_id.in_(coach_ids), Batch.is_active == True)
        .order_by(Batch.name.asc())
        .all()
    )
    for coach_id, name in name_rows:
        batch_names[int(coach_id)].append(name)
    ...
    for coach_id, count in primary_rows:
        if coach_id is not None:
            metrics[int(coach_id)]["primary_students"] = int(count or 0)

    assignment_rows = (
        db.query(
            Assignment.coach_id,
            func.count(Assignment.id),
            func.sum(case((Assignment.is_active == True, 1), else_=0)),
            func.sum(case((Assignment.created_at >= week_ago, 1), else_=0)),
        )
        .filter(Assignment.coach_id.in_(coach_ids))
        .group_by(Assignment.coach_id)
        .all()
    )
    for coach_id, total, active, week_count in assignment_rows:
        cid = int(coach_id)
        metrics[cid]["total_assignments"] = int(total or 0)
        metrics[cid]["active_assignments"] = int(active or 0)
        metrics[cid]["assignments_this_week"] = int(week_count or 0)

    session_rows = (
        db.query(
            Batch.coach_id,
            func.count(ClassSession.id),
            func.sum(case((ClassSession.created_at >= week_ago, 1), else_=0)),
        )
        .join(ClassSession, ClassSession.batch_id == Batch.id)
        .filter(Batch.coach_id.in_(coach_ids))
        .group_by(Batch.coach_id)
        .all()
    )
    for coach_id, total, week_count in session_rows:
        cid = int(coach_id)
        metrics[cid]["sessions_total"] = int(total or 0)
        metrics[cid]["sessions_this_week"] = int(week_count or 0)

    attendance_rows = (
        db.query(
            Attendance.marked_by,
            func.count(Attendance.id),
            func.sum(case((Attendance.marked_at >= week_ago, 1), else_=0)),
        )
        .filter(Attendance.marked_by.in_(coach_ids))
        .group_by(Attendance.marked_by)
        .all()
    )
    for coach_id, total, week_count in attendance_rows:
        cid = int(coach_id)
        metrics[cid]["attendance_marked_total"] = int(total or 0)
        metrics[cid]["attendance_marked_this_week"] = int(week_count or 0)
    ...
    for coach_id, total in announcement_rows:
        metrics[int(coach_id)]["announcements_total"] = int(total or 0)

    total_students_map, active_students_map = _enrolled_student_maps(db, coach_ids)

    return {
        "metrics": metrics,
        "batch_names": batch_names,
        "total_students": total_students_map,
        "active_students": active_students_map,
    }


def _coach_activity_row_from_maps(coach: User, maps: dict) -> CoachActivityRow:
    metrics = maps["metrics"].get(coach.id, {})
    total_students = maps["total_students"].get(coach.id, set())
    active_students = maps["active_students"].get(coach.id, set())
    return CoachActivityRow(
        id=coach.id,
        username=coach.username,
        full_name=coach.full_name or coach.username,
        email=_safe_email(coach),
        is_active=bool(coach.is_active),
        created_at=coach.created_at,
        last_login=coach.last_login,
        days_since_login=_days_since_login(coach.last_login),
        total_batches=int(metrics.get("total_batches", 0)),
        active_batches=int(metrics.get("active_batches", 0)),
        total_students=len(total_students),
        active_students=len(active_students),
        primary_students=int(metrics.get("primary_students", 0)),
        total_assignments=int(metrics.get("total_assignments", 0)),
        active_assignments=int(metrics.get("active_assignments", 0)),
        assignments_this_week=int(metrics.get("assignments_this_week", 0)),
        sessions_total=int(metrics.get("sessions_total", 0)),
        sessions_this_week=int(metrics.get("sessions_this_week", 0)),
        attendance_marked_total=int(metrics.get("attendance_marked_total", 0)),
        attendance_marked_this_week=int(metrics.get("attendance_marked_this_week", 0)),
        announcements_total=int(metrics.get("announcements_total", 0)),
        batch_names=maps["batch_names"].get(coach.id, []),
    )


def _coach_recent_activity(db: Session, coach: User, limit: int = 20) -> List[CoachRecentActivityRow]:
    events: List[CoachRecentActivityRow] = []

    session_rows = (
        db.query(ClassSession, Batch.name)
    ...
    for session, batch_name in session_rows:
        label = session.topic.strip() if session.topic and session.topic.strip() else batch_name
        events.append(
    ...
    assignment_rows = (
        db.query(Assignment)
        .filter(Assignment.coach_id == coach.id)
    ...
    for assignment in assignment_rows:
        events.append(
            CoachRecentActivityRow(
                ...
                label=f"Assignment: {assignment.title}",
    ...
    attendance_rows = (
        db.query(Attendance)
        .filter(Attendance.marked_by == coach.id)
    ...
    for row in attendance_rows:
        events.append(
            CoachRecentActivityRow(
                ...
                occurred_at=row.marked_at,
            )
        )

    announcement_rows = (
        db.query(Announcement)
        .filter(Announcement.created_by == coach.id)
    ...
    for announcement in announcement_rows:
        events.append(
            CoachRecentActivityRow(
                ...
                label=f"Announcement: {announcement.title}",
...
def _coach_activity_batches(db: Session, coach: User) -> List[CoachActivityBatchRow]:
    batches = (
        db.query(Batch)
        .filter(Batch.coach_id == coach.id)
        .order_by(Batch.is_active.desc(), Batch.name.asc())
        .all()
    )
    if not batches:
        return []
    ...
    count_rows = (
        db.query(StudentBatch.batch_id, func.count(StudentBatch.id))
        .filter(StudentBatch.batch_id.in_(batch_ids), StudentBatch.is_active == True)
        .group_by(StudentBatch.batch_id)
        .all()
    )
    counts = {int(batch_id): int(count) for batch_id, count in count_rows}
...
def get_deactivated_notice(
    ...
    return {
        "count": len(rows),
        "students": [
            DeactivatedNoticeStudent(id=u.id, username=u.username, email=_safe_email(u)).model_dump()
            for u in rows
        ],
    }
...
def get_all_students(
    ...
    if not is_admin_user:
        ...
        if unassigned_only:
            ...
            target = _get_assignable_coach(db, coach_id)
            if not target:
                raise HTTPException(status_code=404, detail="Coach not found")
...
def assign_student_to_coach(
    ...
    if payload.coach_id is not None:
        coach_user = _get_assignable_coach(db, payload.coach_id)
        if not coach_user:
            raise HTTPException(status_code=404, detail="Coach not found")
...
def admin_list_coach_activity(
    ...
    """Academy-wide coach activity summary for the admin coaches table."""
    coaches = (
        db.query(User)
    ...
    if not coaches:
        return []
    coach_ids = [c.id for c in coaches]
    maps = _build_coach_activity_maps(db, coach_ids)
    return [_coach_activity_row_from_maps(coach, maps) for coach in coaches]


@admin_router.get("/coaches/{coach_ref}/activity", response_model=CoachDetailedActivity)
def admin_get_coach_activity(
    coach_ref: str,
    admin: User = Depends(require_admin),
    db: Session = Depends(get_db),
):
    """Detailed teaching activity for one coach (admin oversight)."""
    coach = _resolve_coach_ref(db, coach_ref)
    maps = _build_coach_activity_maps(db, [coach.id])
    base = _coach_activity_row_from_maps(coach, maps)
    return CoachDetailedActivity(
        **base.model_dump(),
        batches=_coach_activity_batches(db, coach),
        recent_activity=_coach_recent_activity(db, coach),
    )
